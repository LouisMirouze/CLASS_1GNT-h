...
int background_ncdm_init(
  ...
  for (k=0, filenum=0; k<pba->N_ncdm; k++) {
    ...
    pba->factor_ncdm[k]=pba->deg_ncdm[k]*4*_PI_*pow(pba->T_cmb*pba->T_ncdm[k]*_k_B_,4)*8*_PI_*(pow(pba->lambda_G,2)*_G_)
      /3./pow(_h_P_/pba->varconst_alpha/2./_PI_,3)/pow(_c_,7)*_Mpc_over_m_*_Mpc_over_m_;
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for (n_ncdm=0;n_ncdm<pba->N_ncdm; n_ncdm++) {
        ...
        rho_nu_rel = 56.0/45.0*pow(_PI_,6)*pow(4.0/11.0,4.0/3.0)*(pow(pba->lambda_G,2)*_G_)/pow(_h_P_/pba->varconst_alpha,3)/pow(_c_,7)*
