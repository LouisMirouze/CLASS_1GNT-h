...
int primordial_inflation_find_phi_pivot(
  ...
  if (epsilon > 1.) {
    ...
    case ln_aH_ratio_auto:
      ...
      rho_end = 8*_PI_/3.*rho_end/((pow(pba->lambda_G,2)*_G_)*_h_P_/pba->varconst_alpha/pow(_c_,3))*pow(_Mpc_over_m_,2);
      h = 0.7;
      H0 = h * 1.e5 / _c_;
      rho_c0 = pow(H0,2);
      printf("J'y passe rho end");
      sigma_B = 2. * pow(_PI_,5) * pow(_k_B_,4) / 15. / pow(_h_P_/pba->varconst_alpha,3) / pow(_c_,2);
