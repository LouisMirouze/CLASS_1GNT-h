...
int thermodynamics_workspace_init(
  ...
  ptw->const_NR_numberdens = 2.*_PI_*(_m_e_/_h_P_*pba->varconst_alpha)*(_k_B_/_h_P_*pba->varconst_alpha);
  /* Ionization energy for HI -- temperature equivalent in Kelvin */
  ptw->const_Tion_H = _h_P_/pba->varconst_alpha*_c_*_L_H_ion_/_k_B_;
  /* Ionization energy for HeI -- temperature equivalent in Kelvin */
  ptw->const_Tion_HeI = _h_P_/pba->varconst_alpha*_c_*_L_He1_ion_/_k_B_;
  /* Ionization energy for HeII -- temperature equivalent in Kelvin */
  ptw->const_Tion_HeII = _h_P_/pba->varconst_alpha*_c_*_L_He2_ion_/_k_B_;
...
int thermodynamics_derivs(
  ...
  case recfast:
    ...
    if (ptdw->require_He == _TRUE_) {
      class_call(recfast_dx_He_dz(pth,pba,precfast,x_He,x,x_H,nH,z,Hz,Tmat,Trad,&(dy[ptv->index_ti_x_He])),
                 precfast->error_message,
                 error_message);
    }
