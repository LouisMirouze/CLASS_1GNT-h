...
int input_read_parameters_species(struct file_content * pfc,
  ...
  sigma_B = 2.*pow(_PI_,5.)*pow(_k_B_,4.)/15./pow(_h_P_,3.)*pow(pba->varconst_alpha,3.)/pow(_c_,2);  // [W/(m^2 K^4) = Kg/(K^4 s^3)]
...
int input_default_params(struct background *pba,
  ...
  sigma_B = 2. * pow(_PI_,5) * pow(_k_B_,4) / 15. / pow(_h_P_,3) * pow(pba->varconst_alpha,3) / pow(_c_,2);
