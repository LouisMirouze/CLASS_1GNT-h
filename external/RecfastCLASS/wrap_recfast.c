...
int recfast_init(struct precision* ppr,
  ...
  pre->CDB = _h_P_/pba->varconst_alpha*_c_*(_L_H_ion_-_L_H_alpha_)/_k_B_;
  pre->CDB_He = _h_P_/pba->varconst_alpha*_c_*(_L_He1_ion_-_L_He_2s_)/_k_B_;
  /* Ionization temperatures */
  pre->CB1 = _h_P_/pba->varconst_alpha*_c_*_L_H_ion_/_k_B_;           // equivalent to ptw->const_Tion_H
  pre->CB1_He1 = _h_P_/pba->varconst_alpha*_c_*_L_He1_ion_/_k_B_;     // equivalent to ptw->const_Tion_HeI
  pre->CB1_He2 = _h_P_/pba->varconst_alpha*_c_*_L_He2_ion_/_k_B_;     // equivalent to ptw->const_Tion_HeII
  /* Constants defined for the Peeble's factors  */
  pre->CR = 2.*_PI_*(_m_e_/_h_P_*pba->varconst_alpha)*(_k_B_/_h_P_*pba->varconst_alpha);  // equivalent to ptw->const_NR_numberdens
  pre->CK = pow(Lalpha,3)/(8.*_PI_);
  pre->CK_He = pow(Lalpha_He,3)/(8.*_PI_);
  /* Lyman alpha temperature */
  pre->CL = _c_*_h_P_/pba->varconst_alpha/(_k_B_*Lalpha);
  pre->CL_He = _c_*_h_P_/pba->varconst_alpha/(_k_B_/_L_He_2s_);
  pre->CL_Het = _h_P_/pba->varconst_alpha*_c_*_L_He_2St_/_k_B_;
  /* Helium 2s->2p transition temperature*/
  pre->CDB_He2s2p = _h_P_/pba->varconst_alpha*_c_*(_L_He_2p_-_L_He_2s_)/_k_B_;
 ...
 * */
int recfast_dx_He_dz(struct thermodynamics* pth, struct background* pba, struct thermorecfast * pre, double x_He, double x, double x_H, double nH,
                     double z, double Hz, double Tmat, double Trad,
                     double* dxHe_dz) {
  ...
  /* More difficult cases : Take into account additional contributions */
    ...
    if (Heflag >= 3) {
      ...
      switch (pre->photoion_mode){
        case recfast_photoion_Tmat:
          Rup_trip = Rdown_trip*exp(-_h_P_/pba->varconst_alpha*_c_*_L_He2St_ion_/(_k_B_*Tmat))*pow(pre->CR*Tmat,1.5)*4./3.;
          break;
        case recfast_photoion_Trad:
        default:
          Rup_trip = _a_trip_/(sqrt(Trad/_T_0_)*pow((1.+sqrt(Trad/_T_0_)),(1.-_b_trip_)) * pow((1.+sqrt(Trad/_T_1_)),(1.+_b_trip_))) *exp(-_h_P_/pba->varconst_alpha*_c_*_L_He2St_ion_/(_k_B_*Trad))*pow(pre->CR*Trad,1.5)*4./3.;
          break;
      }
      /* Varying fundamental constants */
      if (pth->has_varconst == _TRUE_) {
      ...
      CL_PSt = _h_P_/pba->varconst_alpha*_c_*(_L_He_2Pt_ - _L_He_2St_)/_k_B_;
